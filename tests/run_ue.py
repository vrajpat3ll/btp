...
def run_ue(ue, start_time, log_dir, use_sudo):
    ...
    cmd = ["kubectl", "-n", ns, "exec", "deploy/sim5g-simulator", "--",
    ...
    try:
        ...
        sig_cmd = ["kubectl", "-n", ns, "exec", "deploy/sim5g-simulator", "--",
    ...
    if proc.poll() is None:
        ...
        del_cmd = ["kubectl", "-n", ns, "delete", "deploy/sim5g-simulator", "--ignore-not-found=true"]
        if use_sudo:
    ...
    if exit_code is None:
        ...
        event["termination_method"] = "graceful" if exit_code == 0 else "deleted-deployment-or-error"
...
def main():
    ...
    p.add_argument("--use-sudo", action="store_true", help="preprend sudo to kubectl calls")
    args = p.parse_args()
    ...
    threads = []
    for ue in ues:
        t = threading.Thread(target=run_ue, args=(ue, start_time, args.log_dir, args.use_sudo), daemon=True)
        t.start()
        threads.append(t)
